...
async def grant_permission(
    ...
    await log_svc.log(
        ...
        f"Berechtigung erteilt: {guest.name} → {machine.name}",
...
async def revoke_permission(
    ...
    await db.commit()
    await log_svc.log(
        db, LogType.permission_revoked,
        f"Berechtigung entzogen: {guest.name if guest else guest_id} → {machine.name if machine else machine_id}",
        guest_id=guest_id, machine_id=machine_id, user_id=current.id
    )
    return {"ok": True}
