...
- **`services/remote_backup.py`**: `_connect()` (Passwort oder Key-Auth per `paramiko`), `upload_file_sync()`, `cleanup_remote_sync()`, `test_connection_sync()` — alles synchron/blockierend, aus async Code immer per `asyncio.to_thread()` aufrufen
...
- **Additiv zur lokalen Aufbewahrung, aber gleiche Aufbewahrungsfrist**: Upload verändert die lokalen Backups nicht, aber `cleanup_remote_sync()` wendet `auto_backup_keep` (dieselbe Einstellung wie lokal, kein eigenes Feld) auch auf das SFTP-Zielverzeichnis an — spiegelt `_cleanup_old_backups()`, sonst würden Remote-Backups unbegrenzt wachsen. Läuft nach `_create_backup()` in `backup_watcher()` (Auto-Backup) und in `POST /backup/files/create` (manuell), nicht beim Restore. Cleanup-Fehler nach erfolgreichem Upload sind nicht fatal (nur `log.warning`) — das eigentliche Ziel (Backup ist offsite) ist bereits erreicht
